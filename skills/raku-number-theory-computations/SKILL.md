...
1. Identify the mathematical operation and look it up in [the routine dictionary](references/functions.json). The dictionary is the supplied package capability index.
...
The test suite is the primary source for executable syntax. The routine dictionary may omit routines exercised by tests, so search the tests when the dictionary has no entry.
